...
后续优先做容器化人工验收、缺陷修复和少量交互打磨。SSH 密码登录、凭据能力增强、真实 SSH 发布和真实企业微信 webhook 发送暂不作为优先阻塞项。
...
当前 SSH 执行器不新增 Go SSH 依赖，使用系统 `ssh` 命令执行私钥发布。密码凭据可以加密保存，但命令后端暂不启用密码登录执行，避免通过不安全的交互式密码注入伪装支持。
