...
function PolicyList({ data, state }: { data: Entity[]; state: AppState }) {
  return <div className="policy-list"><DataList data={data} renderItem={(item) => <div className="policy-row"><div><strong>{formatPolicyScope(item, state)}</strong><small>{`确认方式 · ${item.confirm_mode ?? 'self_confirm'}`}</small></div><div><StatusTag value={item.manual_freeze_enabled === true ? 'frozen' : 'active'} /><small>{item.ssh_realtime_check_required ? 'SSH 实时检查' : '常规预检'}</small></div></div>} /></div>;
}
