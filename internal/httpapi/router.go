...
func NewRouter(deps Dependencies) http.Handler {
	...
	mux.HandleFunc("GET /api/v1/users", withAdmin(store, deps.Config.JWTSecret, listUsers(store)))
	mux.HandleFunc("POST /api/v1/users", withAdmin(store, deps.Config.JWTSecret, createSessionUser(store)))
	mux.HandleFunc("PATCH /api/v1/users/{id}", withAdmin(store, deps.Config.JWTSecret, patchUser(store)))
	...
	mux.HandleFunc("GET /api/v1/release-policies", withAdmin(store, deps.Config.JWTSecret, listReleasePolicies(releases)))
	mux.HandleFunc("POST /api/v1/release-policies", withAdmin(store, deps.Config.JWTSecret, saveReleasePolicy(releases)))
	mux.HandleFunc("GET /api/v1/release-policies/effective", withOptionalAPIKeyScope(store, "release:read", getEffectiveReleasePolicy(releases)))
	mux.HandleFunc("POST /api/v1/release-policies/freeze", withAdmin(store, deps.Config.JWTSecret, freezeReleasePolicy(releases, true)))
	mux.HandleFunc("POST /api/v1/release-policies/unfreeze", withAdmin(store, deps.Config.JWTSecret, freezeReleasePolicy(releases, false)))
