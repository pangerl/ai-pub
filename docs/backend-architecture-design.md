...
- Mock/Dry-run 和 SSH 执行器遵守同一 contract。
...
6. 展开目标服务器，创建 `ServerDeployLog(queued)`。
7. 冻结部署目标执行快照。
8. 写入确认和入队事件。
...
3. 必要时更新 `ServerDeploymentState`。
4. 写入 `server_finished` 事件。
