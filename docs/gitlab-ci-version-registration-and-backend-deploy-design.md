...
### 2.2 服务必须预先存在，CI 只登记版本

CI 不传展示名称 `project` 或 `service` 来匹配、更不允许自动创建实体。每个 GitLab 项目在受保护变量中配置固定的 `AI_PUB_SERVICE_ID`，发布系统据此找到既有服务并推导项目归属。

这样避免服务改名、同名服务、错误项目路径或恶意回调导致的错误归属。服务的创建、启用和部署目标配置继续由管理员在发布系统完成。
...
```text
POST /api/v1/services/{service_id}/versions
```

同一路径按调用身份区分：管理员会话用于手动登记；带 `version:write` scope 的 API Key 用于 CI 登记。CI 调用时服务端强制写入 `source=ci`、`created_by_type=api_key` 和调用方 ID，不能由请求体伪造。

需要新增 `version:write` API Key scope。它只允许登记版本，不可创建服务、修改部署目标或执行发布；GitLab 项目只保存其专用且受保护、掩码显示的 API Key。
...
其中 `version`、`commit_sha`、`artifact_url`、`pipeline_id`、`pipeline_url` 必填。`ref`、`gitlab_project_path`、`built_at` 可选。
  ...
  script:
    ...
    - |
      payload=$(jq -n \
        ...
        '{version:$version, commit_sha:$commit_sha, artifact_url:$artifact_url, pipeline_id:$pipeline_id, pipeline_url:$pipeline_url, ref:$ref, gitlab_project_path:$project_path, built_at:$built_at}')
      curl --fail-with-body --retry 5 --retry-all-errors \
        -X POST "${AI_PUB_BASE_URL}/api/v1/services/${AI_PUB_SERVICE_ID}/versions" \
...
- `AI_PUB_SERVICE_ID`、`AI_PUB_API_KEY`、`AI_PUB_BASE_URL` 是 GitLab 受保护变量；不得写入仓库。
