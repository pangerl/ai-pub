...
状态：已完成。已通过一台非生产服务器的密码认证、脚本可执行性检查和真实发布验证。
...
- 真实 SSH 密码登录、`/home/dm/service/deploy.sh` 执行、退出码和服务器日志已在非生产测试服务器验证为 success。
...
- 用户通过 Web 完成一次发布。
- 管理员通过 Web 完成部署目标和环境发布保护配置。
- 状态标签、warning、block、partial 展示正确。

### M6 通知和运维增强

状态：已完成。真实企业微信机器人 webhook 测试发送已验证成功。
...
- 运行摘要可定位队列、执行器和通知问题。
- 企业微信机器人真实测试发送返回 success，投递记录为 `sent`。
...
- 真实 SSH 发布与企业微信 webhook 发送均已完成测试验证；后续改动仍须保持本地 MVP 验收通过。
