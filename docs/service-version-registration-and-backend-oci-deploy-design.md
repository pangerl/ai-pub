...
- 在版本登记接口中隐式创建、确认或执行发布单。若采用持续部署模式，CI 应在登记成功后显式调用通用发布 API，并继续遵守 preflight、scope、生产确认和审计边界。
...
本示例用于后端 OCI 镜像：版本登记 Job 排在镜像 push 成功之后。示例采用持续交付模式，CI 只负责登记制品，不运行服务器部署脚本，也不创建发布单。若团队采用持续部署模式，可在本 Job 成功后使用另一 Job 调用通用 preflight、创建和确认 API；真实部署仍由 ai-pub Worker 执行，CI 不持有 SSH 或 K8s 部署凭据，生产发布仍等待管理员确认。其他 CI 复用相同边界即可。
