...
本地 MVP 已完成，可作为当前基线冻结。运行时只支持 MySQL 8；Docker Compose 从空数据库启动 MySQL、Go 后端、Nginx 前端和一次性验证容器。MySQL 与后端不映射宿主机端口，前端地址为 `http://127.0.0.1:18080/`。

本次审查的自动验证均通过，因此“已完成”仅指下表列出的本地 MVP 闭环；不把尚未纳入本地自动验收的扩展能力算入完成范围。
