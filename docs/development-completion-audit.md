...
本地 MVP 已完成，可作为上线准备基线。运行时只支持 MySQL 8；Docker Compose 从空数据库启动 MySQL、单应用容器和一次性验证容器。单应用容器同时提供 SPA 静态资源、REST API、启动迁移和内置 Worker；MySQL 不映射宿主机端口，应用地址为 `http://127.0.0.1:18080/`。
...
| 范围 | 状态 | 证据 |
| --- | --- | --- |
| MySQL 8 容器运行与 migration | 已完成 | `make compose-check` 从空库通过 |
...
| 前端与日常操作界面 | 已完成 | Go 应用容器提供 SPA 静态资源和 `/api/v1` 接口；发布、配置、系统与个人访问密钥入口已实现 |
